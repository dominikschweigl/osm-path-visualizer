...
export const useSearchAddress = (): UseSearchAddressResult => {
  ...
  const [results, setResults] = useState<Record<string, SearchResult<RawResult>[]>>({});
  const [loading, setLoading] = useState(false);
  const [selectedItem, setSelectedItem] = useState<SearchResult<RawResult> | null>(null);

  const debouncedQuery = useDebounce(query, 500);

  const groupByType = useCallback((data: SearchResult<RawResult>[]): Record<string, SearchResult<RawResult>[]> => {
    return data.reduce((acc, item) => {
      const { raw } = item;

      const rawData = raw as unknown as any;
      const type = rawData.addresstype;

      if (!LOCATION_TYPES.includes(type)) return acc;
  ...
  useEffect(() => {
    const fetchResults = async () => {
      if (debouncedQuery.length > 2) {
        setLoading(true);
        const provider = new OpenStreetMapProvider({ params: { "accept-language": "en", addressdetails: 1 } });
